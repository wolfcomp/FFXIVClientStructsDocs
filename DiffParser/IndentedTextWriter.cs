...
internal sealed class IndentedTextWriter : IDisposable {
    ...
    public void Write(string content, bool isMultiline = false, char? diff = null) {
        Write(content.AsSpan(), isMultiline, diff);
    }
    ...
    /// <param name="content">The content to write.</param>
    /// <param name="isMultiline">Whether the input content is multiline.</param>
    public void Write(ReadOnlySpan<char> content, bool isMultiline = false, char? diff = null) {
        if (isMultiline) {
    ...
    /// <param name="content">The content to write.</param>
    /// <param name="isMultiline">Whether the input content is multiline.</param>
    public void WriteIf(bool condition, string content, bool isMultiline = false, char? diff = null) {
        if (condition) {
            Write(content.AsSpan(), isMultiline, diff);
        }
    }
    ...
    /// <param name="content">The content to write.</param>
    /// <param name="isMultiline">Whether the input content is multiline.</param>
    public void WriteIf(bool condition, ReadOnlySpan<char> content, bool isMultiline = false, char? diff = null) {
        if (condition) {
            Write(content, isMultiline, diff);
        }
    }
    ...
    /// <param name="content">The content to write.</param>
    /// <param name="isMultiline">Whether the input content is multiline.</param>
    public void WriteLine(string content, bool isMultiline = false, char? diff = null) {
        WriteLine(content.AsSpan(), isMultiline, diff);
    }
    ...
    /// <param name="content">The content to write.</param>
    /// <param name="isMultiline">Whether the input content is multiline.</param>
    public void WriteLine(ReadOnlySpan<char> content, bool isMultiline = false, char? diff = null) {
        Write(content, isMultiline, diff);
        WriteLine();
    }
    ...
    /// <param name="content">The content to write.</param>
    /// <param name="isMultiline">Whether the input content is multiline.</param>
    public void WriteLineIf(bool condition, ReadOnlySpan<char> content, bool isMultiline = false, char? diff = null) {
        if (condition) {
            Write(content, isMultiline, diff);
            WriteLine();
        }
    ...
    private void WriteRawText(ReadOnlySpan<char> content, char? diff = null) {
        if (diff.HasValue) builder.Add(diff.Value);
        if (builder.Count == 0 || builder.WrittenSpan[^2..^1].SequenceEqual(DefaultNewLine.AsSpan())) {
            builder.AddRange(currentIndentation.AsSpan());
        }
    ...
    public readonly ref struct WriteInterpolatedStringHandler {
        ...
        public void AppendLiteral(string value, char? diff = null) {
            writer.Write(value, diff: diff);
        }

        /// <summary>Writes the specified value to the handler.</summary>
        /// <param name="value">The value to write.</param>
        public void AppendFormatted(string? value, char? diff = null) {
            AppendFormatted<string?>(value, diff);
        }

        /// <summary>Writes the specified character span to the handler.</summary>
        /// <param name="value">The span to write.</param>
        public void AppendFormatted(ReadOnlySpan<char> value, char? diff = null) {
            writer.Write(value, diff: diff);
        }
        ...
        public void AppendFormatted<T>(T value, char? diff = null) {
            if (value is not null) {
                writer.Write(value.ToString(), diff: diff);
            }
        }
        ...
        /// <typeparam name="T">The type of the value to write.</typeparam>
        public void AppendFormatted<T>(T value, string? format, char? diff = null) {
    ...
    public readonly ref struct WriteIfInterpolatedStringHandler {
        ...
        /// <inheritdoc cref="WriteInterpolatedStringHandler.AppendLiteral(string, char?)" />
        public void AppendLiteral(string value, char? diff = null) {
            handler.AppendLiteral(value, diff);
        }

        /// <inheritdoc cref="WriteInterpolatedStringHandler.AppendFormatted(string?, char?)" />
        public void AppendFormatted(string? value, char? diff = null) {
            handler.AppendFormatted(value, diff);
        }

        /// <inheritdoc cref="WriteInterpolatedStringHandler.AppendFormatted(ReadOnlySpan{char}, char?)" />
        public void AppendFormatted(ReadOnlySpan<char> value, char? diff = null) {
            handler.AppendFormatted(value, diff);
        }

        /// <inheritdoc cref="WriteInterpolatedStringHandler.AppendFormatted{T}(T, char?)" />
        public void AppendFormatted<T>(T value, char? diff = null) {
            handler.AppendFormatted(value, diff);
        }

        /// <inheritdoc cref="WriteInterpolatedStringHandler.AppendFormatted{T}(T, string?, char?)" />
        public void AppendFormatted<T>(T value, string? format, char? diff = null) {
            handler.AppendFormatted(value, format, diff);
        }
    }
}
